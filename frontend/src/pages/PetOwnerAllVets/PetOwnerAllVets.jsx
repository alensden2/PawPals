...
const PetOwnerAllVets = () => {
  ...
  const handleBookClick = (vetUserId) => {
    ...
    if (user && user.userName) {
      getAllAnimals(user.userName).then((allAnimals) => {
        setPetOptions(
          allAnimals.map((animal) => ({ label: animal.name, value: animal.id }))
        );
        setModalState((prevState) => {
          return {
            ...prevState,
            isOpen: true,
            vet: vetsState.find((v) => v.userName === vetUserId)
          };
        });
      });
    }
      ...
      bookAppointment(appointmentObj)
        .then(() => {
          setToast({
            type: 'success',
            message:
              'Appointment is requested, please wait for confirmation email from the vet'
          });
        })
        .catch(() => {
          setToast({
  ...
  const handleDateChange = async (newValue) => {
    ...
    if (modalState.vet) {
      const response = await getVetAvailabilityOnSpecificDay(
        selectedDate,
        modalState.vet.userName
      );
      setDateOptions(
        response.slots.map((slot) => ({
          value: `${slot.first}-${slot.second}`
        }))
      );
    }
  ...
  return (
    <>
      ...
      <Modal open={modalState.isOpen} onClose={onModalClose}>
        <div className={classes.paper}>
          <h3 id="simple-modal-title">
            You&apos;re booking appointment with:{' '}
            {modalState.vet?.firstName + ' ' + modalState.vet?.lastName || ''}
          </h3>
          <div>
            ...
            <Select
              ...
              onChange={(event) => {
                const petOp = petOptions.find(
                  (petOption) => event.target.value === petOption.value
                );
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            ...
            <DatePicker value={dateSelected} onChange={handleDateChange} />
