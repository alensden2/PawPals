...
public void testUpdatedPetOwner() throws InvalidPetOwnerObject, UserNameNotFound, InvalidImage, IOException {
    PetOwnerDto petOwnerDto = new PetOwnerDto();
    petOwnerDto.setAddress("new address");
    MultipartFile image = new MockMultipartFile("image", "test-image.jpg", "image/jpeg", new byte[10]);

    String id = "12";
    when(petOwnerServiceMock.updatePetOwner(eq(id), eq(petOwnerDto),any(MultipartFile.class))).thenReturn(new PetOwnerDto());
    Object requestBody = petOwnerDto;
    ResponseEntity<ApiResponse> response = petOwnerController.updatePetOwner(id,requestBody,image);

    verify(petOwnerServiceMock).updatePetOwner(eq(id), eq(petOwnerDto),any(MultipartFile.class));
