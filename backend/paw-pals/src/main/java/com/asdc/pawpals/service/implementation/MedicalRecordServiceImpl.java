...
public class MedicalRecordServiceImpl implements MedicalRecordService {
   ...
   * @return List of MedicalHistoryDto objects representing the medical history
   *         records for the given animal ID
   */
  @Override
  public List<MedicalHistoryDto> retrieveMedicalRecord(Long animalId) {
    ...
    if (medicalRecords != null && !medicalRecords.isEmpty()) {
      medicalHistoryDto = medicalRecords
          .stream()
   ...
   *
   * @param medicalHistoryDto the MedicalHistoryDto object containing the medical
   *                          record information.
   ...
   * @throws UserNameNotFound       if the Vet user ID provided in the
   *                                MedicalHistoryDto does not exist in the
   *                                database.
   * @throws InvalidAnimalId        if the animal ID provided in the
   *                                MedicalHistoryDto does not exist in the
   *                                database.
   */
  @Override
  public MedicalHistoryDto postMedicalRecord(
    ...
    logger.debug(
        "Book pet owner appointment with vet %s",
        medicalHistoryDto);
    /**
     * old Code -
     * if (
    ...
    if(medicalHistoryDto != null){
      boolean isAilmentNameNotNull = medicalHistoryDto.getAilmentName() != null;
      boolean isPrescriptionNotNull = medicalHistoryDto.getPrescription() != null;
      boolean isDateDiagnosedNotNull = medicalHistoryDto.getDateDiagnosed() != null;
      boolean isVaccinesNotNull = medicalHistoryDto.getVaccines() != null;
  
      if (isAilmentNameNotNull &&
        ...
        MedicalHistory medicalHistory = Transformations.DTO_TO_MODEL_CONVERTER.medicalHistory(
            medicalHistoryDto);
        medicalHistory.setVet(optionalVetRepository.get()); // this needs to be done because CascadeType.MERGE is not
                                                            // working with vet->USer->userId
